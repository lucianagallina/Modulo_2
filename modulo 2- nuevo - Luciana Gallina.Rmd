...
**Consigna 1:** Las últimas 3 cifras de mi DNI son 791. Crear una variable que tenga ese número.

```{r}
DNI=791
```

**Consigna 2:** Crear un vector que tenga todos los números desde el 1 hasta el 791.
```{r}
for(i in 1:791) 
print(1:i)
```
...
{python}
secuenciaDNI = range(1, 792)  # incluye 791
...
ggplot(df, aes(x = expr, y = time_ms, fill = expr)) +
  geom_violin(trim = FALSE, alpha = 0.8, color = "lightblue") +
